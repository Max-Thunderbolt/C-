...
const string EVENTTYPE[9] = {"Meeting", "Birthday Reminder", "Party", "Semester Test", "Class Test", "Exam", "Lecture", "Booking", "Study Session"};
...
struct event_data
{
    ...
    template <typename T>
    long int to_long(T input);

    string unix_to_string(long int timestamp);
...
static string unix_to_string(long int timestamp)
{
    ...
    stringstream ss;
    ss << to_string(year) << "/" << ((month < 10) ? "0" + to_string(month) : to_string(month)) << "/" << ((day < 10) ? "0" + to_string(day) : to_string(day)) << " ";
    ss << ((hour < 10) ? "0" + to_string(hour) : to_string(hour)) << ":";
    ss << ((minutes < 10) ? "0" + to_string(minutes) : to_string(minutes)) << ":" << ((seconds < 10) ? "0" + to_string(seconds) : to_string(seconds));
    return ss.str();
}
